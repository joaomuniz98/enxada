...
export default function Scenario({ children }: ScenarioProps) {
  return (
    <>
      <img className="absolute pointer-events-none bg-[url('/poco-hero.png')] -z-10 filter brightness-[15%]  saturate-200 mix-blend-multiply bg-blend-multiply bg-origin-border left-0 right-0 bg-contain h-full [background-position-y:_top] [background-position-x:_center] bg-no-repeat bg-fixed" alt="Imagem do poço" />
      {children}
